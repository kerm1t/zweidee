  ...
  void RenderThread(void *args)
  {
    ...
    while (true)
    {
      ...
      if (accumulatedTimeSinceLastUpdate > 12) // indep. from gfx-card -> update every 12 [ms]
      {
        ...
        if (GetAsyncKeyState(VK_UP))    call_up();// m_engine.up();
        if (GetAsyncKeyState(VK_DOWN))  call_down();//m_engine.down();
        if (GetAsyncKeyState(VK_LEFT))  call_left();//m_engine.left();
        if (GetAsyncKeyState(VK_RIGHT)) call_right();//m_engine.right();

        call_doit();// m_engine.move();
      }
